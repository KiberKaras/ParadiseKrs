...
/obj/structure/terrain_scanner/attack_hand(mob/user)
	switch(tgui_alert(user, "Что вы собираетесь сделать?", "Сканер местности", list("Уничтожить все объекты из черного списка", "Получить список уникальных объектов", "Получить карту определенного объекта", "Получить карту стен и полов", "Получить полный набор карт")))
